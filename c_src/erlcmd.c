...
void erlcmd_send(char *response, size_t len)
{
    ...
    size_t wrote = 0;
    do {        
        ssize_t amount_written = write(STDOUT_ALTER_FILENO, response + wrote, len - wrote);
        if (amount_written < 0) {
            if (errno == EINTR)
                continue;
...
#else
    ssize_t amount_read = read(STDIN_ALTER_FILENO, handler->buffer + handler->index, sizeof(handler->buffer) - handler->index);
    if (amount_read < 0) {
